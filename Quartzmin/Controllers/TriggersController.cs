        ...
        public async Task<IActionResult> Index()
        {
            ...
            foreach (var key in keys)
            {
                ...
                list.Add(new TriggerListItem()
                {
                    ...
                    StartTime = t.StartTimeUtc.UtcDateTime.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                    //StartTime = t.StartTimeUtc.UtcDateTime.ToDefaultFormat(),
                    EndTime = t.FinalFireTimeUtc?.UtcDateTime.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                    //EndTime = t.FinalFireTimeUtc?.UtcDateTime.ToDefaultFormat(),
                    LastFireTime = t.GetPreviousFireTimeUtc()?.UtcDateTime.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                    //LastFireTime = t.GetPreviousFireTimeUtc()?.UtcDateTime.ToDefaultFormat(),
                    NextFireTime = t.GetNextFireTimeUtc()?.UtcDateTime.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
