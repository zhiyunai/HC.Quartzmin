        ...
        public async Task<IActionResult> Index()
        {
            ...
            foreach (var h in history.OrderByDescending(x => x.ActualFireTimeUtc))
            {
                ...
                list.Add(new
                {
                    ...
                    ScheduledFireTimeUtc = h.ScheduledFireTimeUtc?.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                    //ScheduledFireTimeUtc = h.ScheduledFireTimeUtc?.ToDefaultFormat(),
                    ActualFireTimeUtc = h.ActualFireTimeUtc.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                    //ActualFireTimeUtc = h.ActualFireTimeUtc.ToDefaultFormat(),
                    FinishedTimeUtc = h.FinishedTimeUtc?.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
